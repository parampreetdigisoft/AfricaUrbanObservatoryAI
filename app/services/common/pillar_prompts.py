...
Enhanced with African Urban Index pillar-specific prompts
"""
...
class PillarPrompts:
    """African Urban Index pillar-specific prompt templates"""
    ...
    def get_pillar_catalog_for_live_feed(cls) -> str:
        """Compact AUI pillar catalog for live African pillar signals."""
        lines = []
    ...
    def pillar_live_signals_prompt(cls) -> str:
        ...
        return f"""
        You are the African Urban Index (AUI) live pillar intelligence engine.

        Produce a LIVE African snapshot: exactly ONE card per AUI pillar (IDs 1–14).
        Use the pillar definitions below to ground each card in the correct domain.
        Every signal MUST concern an African city, African country, or African urban system.
        Do not use non-African cities (London, Singapore, New York, Dubai) as the primary signal.

        ==================================================
        AUI PILLAR CATALOG (ALL 14 — MANDATORY COVERAGE)
        ==================================================
        {catalog}
        ...
        ==================================================
        Before writing JSON, search credible African and international news for each pillar domain.
        For each pillar, find the most relevant African urban signal from the LAST 48 HOURS.
        Older context only if an actively developing trend requires brief background
        (same rules as AUI live country feed).

        ==================================================
        sourceUrl RULES
        ==================================================
        - One HTTPS URL per pillar, copied exactly from search OR Google News search:
          https://news.google.com/search?q=AFRICAN+CITY+PILLAR+TOPIC+KEYWORDS&hl=en&gl=ZA&ceid=ZA:en
        ...
        - summary: max 100 characters — one clear African urban signal for this pillar.
        ...
        {{
            ...
            "headline": "Live African Pillar Signals",
            "subHeadline": "African Urban Index pillar watch from the last 48 hours.",
            "pillars": [
                {{
                    "pillarId": 1,
                    "type": "risk",
                    "title": "Short headline",
                    "summary": "One sentence African urban signal for this pillar domain.",
                    "status": "Watch",
                    "urgency": "medium",
                    "color": "yellow",
                    "sourceUrl": "https://news.google.com/search?q=african+cities+sanitation+waste&hl=en&gl=ZA&ceid=ZA:en"
                }}
            ]
        }}
