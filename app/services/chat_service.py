...
class ChatService:
    ...
    Prompt text lives in VerdianPromptTemplates (African Urban Index).
    """
...
# ============================================================
    ...
    def _normalize_emerging_trends_payload(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "subHeadline": str(
                data.get(
                    "subHeadline",
                    "Live African urban signals from the last 24 hours across governance, security, economy, and society.",
                )
            ).strip(),
    ...
    def _normalize_pillar_live_signals_payload(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "updatedAt": str(updated_at),
            "headline": str(data.get("headline", "Live African Pillar Signals")).strip(),
            "subHeadline": str(
                data.get(
                    "subHeadline",
                    "African Urban Index pillar watch from the last 48 hours.",
                )
            ).strip(),
            "pillars": pillars,
        }
